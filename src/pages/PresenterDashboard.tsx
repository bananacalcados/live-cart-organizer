...
const alertTypeConfig: Record<string, { label: string; color: string; icon: typeof Bell }> = {
  ...
  team_message: { label: "📢 Recado da Equipe", color: "bg-fuchsia-500/20 text-fuchsia-400 border-fuchsia-500/30", icon: MessageCircle },
      ...
      <div className="mb-6 rounded-2xl border-2 border-amber-500 bg-zinc-950 p-3 shadow-lg">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2 px-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold uppercase tracking-widest text-zinc-400 md:text-base">
              Ainda não clicaram no WhatsApp
            </h2>
          </div>
          <span className="rounded-full border border-amber-500/20 bg-amber-500/10 px-2.5 py-0.5 text-xs font-black text-amber-500">
        ...
        {noClickLeads.length === 0 ? (
          <p className="py-2 text-center text-xs font-semibold text-emerald-400">
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4 lg:grid-cols-6">
            {noClickLeads.map((lead) => (
              <div
                key={lead.id}
                className="flex flex-col items-center justify-center rounded-lg border-b-4 border-amber-600 bg-amber-400 px-2 py-1.5 shadow transition-transform hover:scale-[1.03]"
              >
                <span className="mb-0.5 w-full truncate text-center text-[8px] font-black uppercase tracking-tight text-amber-900/80">
                  {formatTime(lead.created_at)}
                  {lead.instagram && lead.name ? ` · ${lead.name.split(" ")[0]}` : ""}
                </span>
                <span className="w-full truncate text-center text-base font-black tracking-tight text-black md:text-lg xl:text-xl">
                  {lead.instagram
