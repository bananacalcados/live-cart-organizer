...
export function CampaignAudienceManager() {
  ...
  const openNew = () => {
    setEditing({ id: "", nome: "", filtro_json: {} });
    setName("");
    setFilter(emptyAudienceFilter());
  };

  const openEdit = (row: PublicoRow) => {
    setEditing(row);
    setName(row.nome);
  ...
  const save = async () => {
    ...
    let error;
    if (editing?.id) {
      ({ error } = await supabase.from("campanha_publicos").update(payload).eq("id", editing.id));
    } else {
      ({ error } = await supabase.from("campanha_publicos").insert(payload));
    }
    setSaving(false);
    if (error) {
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await supabase.from("campanha_publicos").delete().eq("id", id);
    if (error) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-500">
            Monte públicos reutilizáveis combinando filtros de inclusão e exclusão. Depois selecione-os ao criar uma automação.
          </p>
        </div>
