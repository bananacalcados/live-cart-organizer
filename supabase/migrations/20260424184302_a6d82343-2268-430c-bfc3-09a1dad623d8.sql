-- 1) Cria entrada no Vault (placeholder; valor real será definido logo abaixo via insert tool)
DO $$
DECLARE
  v_existing_id uuid;
BEGIN
  SELECT id INTO v_existing_id FROM vault.secrets WHERE name = 'meta_capi_internal_secret';
  IF v_existing_id IS NULL THEN
    PERFORM vault.create_secret('PLACEHOLDER_REPLACE_ME', 'meta_capi_internal_secret', 'Internal shared secret used by trigger_meta_capi_offline_purchase to call the meta-capi-offline edge function');
  END IF;
END $$;

-- 2) Atualiza a trigger pra ler o secret do Vault
