  ...
  reg_nb AS (
    ...
    WHERE r.catalog_page_id = v_catalog_page_id
      AND (r.checkout_sale_id IS NULL
           OR NOT EXISTS (SELECT 1 FROM public.pos_sales ps WHERE ps.id = r.checkout_sale_id
  ...
  non_buyers_dedup AS (
    SELECT DISTINCT ON (u.pkey) u.pkey,
      ...
      CASE WHEN r.status='checkout_started' THEN 'checkout_started'
           ...
           WHEN r.id IS NOT NULL THEN 'registered_only' ELSE 'lead_only' END AS reason,
      COALESCE(r.updated_at, l.created_at) AS last_activity_at,
      l.source AS lead_source
    FROM (SELECT pkey, updated_at AS last_activity_at FROM reg_nb
          UNION SELECT pkey, created_at FROM lead_nb) u
  ...
  classified AS (
    SELECT e.*, COALESCE(p.prior_count,0) AS prior_sales, p.first_prior_at,
      ...
      CASE
        ...
        WHEN e.kind='buyer' AND (lp.tags IS NOT NULL OR es.srcs IS NOT NULL OR pr.pkey IS NOT NULL OR cc.pkey IS NOT NULL OR zp.pkey IS NOT NULL OR c.customer_id IS NOT NULL)
        THEN 'lead_first_purchase'
        WHEN e.kind='non_buyer' AND (lp.tags IS NOT NULL OR es.srcs IS NOT NULL OR pr.pkey IS NOT NULL OR cc.pkey IS NOT NULL OR zp.pkey IS NOT NULL OR c.customer_id IS NOT NULL)
        THEN 'lead_first_purchase'
        ELSE 'brand_new'
      END AS bucket
    FROM everyone e
  ...
  buyer_rows AS (
    SELECT jsonb_build_object(
      ...
      'sources', jsonb_build_object(
        ...
        'in_chat_contacts', in_chat, 'in_zoppy', in_zoppy
      )
    ) AS row, bucket, value
