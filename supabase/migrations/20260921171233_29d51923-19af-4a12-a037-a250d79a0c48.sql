DROP FUNCTION IF EXISTS public.get_relatorio_grade_live(uuid, text, boolean);
...
CREATE OR REPLACE FUNCTION public.get_relatorio_grade_live(p_live_id uuid, p_status text DEFAULT 'pago'::text, p_incluir_anterior boolean DEFAULT false)
 RETURNS TABLE(produto_nome text, cor text, total_vendido integer, grades integer, status text, vendidos jsonb, vender_mais jsonb, tamanhos_estouro text[], grade_cheia boolean, tamanhos_fora_da_grade jsonb, estoque jsonb, estoque_total integer, comprar jsonb, grades_comprar integer)
 LANGUAGE sql
 STABLE
...
-- liga cada (produto, cor) da live ao cadastro do PDV, via sku ou código de barras
mapa as (
  select distinct i.produto_nome, i.cor, p.name as pname, p.color as pcolor
  from itens i
  join pos_products p on p.sku = i.sku or p.barcode = i.sku
  where i.sku is not null and i.produto_nome <> '' and i.cor <> ''
),
estoque_tam as (
  select m.produto_nome, m.cor, trim(coalesce(p.size,'')) as tam,
         greatest(0, sum(coalesce(p.stock,0)))::int as qtd
  from mapa m
  join pos_products p on p.name = m.pname and coalesce(p.color,'') = coalesce(m.pcolor,'')
  group by 1,2,3
),
padrao as (
...
niveis as (
  select
    ...
    greatest(0, coalesce(p.qtd,0) - coalesce(e.qtd,0)) as falta,
    coalesce(n.n,0)   as n,
  ...
  left join estoque_tam e on e.produto_nome=gp.produto_nome and e.cor=gp.cor and e.tam=gr.tam
...
por_grupo as (
  select
    ...
    sum(estoque)::int as estoque_total,
    coalesce(max(ceil(falta::numeric / grade_qtd))::int, 0) as n_comprar,
    ...
    coalesce(jsonb_agg(jsonb_build_object('tam',tam,'qtd',estoque)
             order by tam) filter (where estoque > 0), '[]'::jsonb) as estoque,
    coalesce(jsonb_agg(jsonb_build_object('tam',tam,'qtd',falta)
             order by tam) filter (where falta > 0), '[]'::jsonb) as comprar,
...
final as (
  select
    ...
    pg.estoque_total,
    pg.comprar,
    pg.n_comprar as grades_comprar
  from por_grupo pg
