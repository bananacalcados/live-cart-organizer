-- Fix #1: premature completion race.
-- A dispatch must only be finalized as 'completed' once ALL recipients have been
-- inserted (count of rows >= total_recipients). Otherwise, while the frontend is
-- still inserting recipients in concurrent batches, the engine can see an empty
-- pending set for a moment and close the dispatch, orphaning the rest.
-- A 15-minute grace window prevents a dispatch from getting stuck forever if some
-- recipient inserts failed (count < total).
...
BEGIN
  WITH done AS (
    ...
    WHERE d.status = 'sending'
      -- All recipients fully inserted, OR a safety grace window elapsed.
      AND (
...
-- Fix #2: bulk-mark sent recipients in a single statement to reduce the
-- connection-pool storm caused by per-row UPDATEs in the worker.
