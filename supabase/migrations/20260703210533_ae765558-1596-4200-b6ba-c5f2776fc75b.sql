...
-- 1) Novas colunas em pos_sales (nullable, sem impacto no existente)
ALTER TABLE public.pos_sales
  ADD COLUMN IF NOT EXISTS is_conditional boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS conditional_status text,
  ADD COLUMN IF NOT EXISTS conditional_signed_at timestamptz;

CREATE INDEX IF NOT EXISTS idx_pos_sales_conditional
  ON public.pos_sales (conditional_status)
  WHERE is_conditional = true;

-- 2) process_pos_sale_sale_event: aceitar status 'conditional' na guarda de entrada
...
BEGIN
  ...
  LOOP
    ...
    WHERE store_id = v_target_store
      AND (
        barcode = ANY(v_codes) OR sku = ANY(v_codes)
        OR (v_alias_skus IS NOT NULL AND sku = ANY(v_alias_skus))
        OR (v_alias_tiny IS NOT NULL AND tiny_id = ANY(v_alias_tiny))
      )
      AND COALESCE(stock, 0) >= v_item.quantity
    ORDER BY (CASE WHEN barcode = ANY(v_codes) THEN 0 ELSE 1 END)
    LIMIT 1;
    ...
    IF v_product.id IS NULL THEN
      ...
      WHERE (
        ...
        AND COALESCE(stock, 0) >= v_item.quantity
      ORDER BY COALESCE(stock, 0) DESC, (CASE WHEN barcode = ANY(v_codes) THEN 0 ELSE 1 END)
      LIMIT 1;
    END IF;
...
-- 3) apply_pos_sale_stock_movement: baixar no INSERT tambem quando 'conditional';
--    e no cancelamento nao restaurar itens que ja foram devolvidos (sale_event='return')
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.status IN ('completed', 'paid', 'conditional') THEN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF NEW.status = 'cancelled' AND COALESCE(OLD.status, '') <> 'cancelled' THEN
      ...
      LOOP
        IF EXISTS (
          SELECT 1 FROM public.pos_stock_adjustments
          WHERE sale_id = NEW.id AND product_id = v_adj.product_id
            AND sale_event IN ('cancel', 'return')
        ) THEN
          CONTINUE;
        END IF;
...
-- 4) restore_pos_sale_item_stock: restaura o estoque de um item devolvido na finalizacao
...
BEGIN
  ...
  LOOP
    -- guard contra restauracao dupla
    IF EXISTS (
      SELECT 1 FROM public.pos_stock_adjustments
    ...
    INSERT INTO public.pos_stock_adjustments(
      store_id, product_id, tiny_id, sku, barcode, product_name,
      direction, quantity, previous_stock, new_stock, reason,
      sale_id, sale_event, tiny_mirror_status
    ) VALUES (
      v_adj.store_id, v_adj.product_id, v_adj.tiny_id, v_adj.sku, v_adj.barcode,
      COALESCE(v_adj.product_name, ''),
    ...
    IF v_adj.tiny_id IS NOT NULL THEN
      PERFORM net.http_post(
        ...
        body    := jsonb_build_object(
          'sale_id', p_sale_id, 'store_id', v_adj.store_id, 'sale_event', 'cancel',
          'items', jsonb_build_array(jsonb_build_object(
            'adjustment_id', v_adj_id, 'product_id', v_adj.product_id, 'store_id', v_adj.store_id,
            'tiny_id', v_adj.tiny_id, 'sku', v_adj.sku, 'new_stock', v_new,
            'quantity', v_adj.quantity, 'direction', 'in'
          ))
        )
      );
    END IF;
    ...
    PERFORM net.http_post(
      ...
      headers := jsonb_build_object('Content-Type','application/json','apikey', v_anon_key,'Authorization','Bearer '||v_anon_key),
      body    := jsonb_build_object(
        'sale_id', p_sale_id, 'sale_event', 'cancel',
        'items', jsonb_build_array(jsonb_build_object('barcode', v_adj.barcode, 'sku', v_adj.sku))
      )
    );
  END LOOP;
END;
...
GRANT EXECUTE ON FUNCTION public.restore_pos_sale_item_stock(uuid, text, text) TO service_role;
