...
BEGIN
  ...
  WITH sold AS (
    SELECT DISTINCT ON (s.store_id, trim(si.sku))
      ...
      si.product_name,
      si.variant_name,
      trim(coalesce(si.barcode,'')) AS barcode,
      si.tiny_product_id,
      si.unit_price,
      si.category,
      si.created_at
  ...
  FOR r IN SELECT * FROM _phantoms LOOP
    v_sku := r.sku;
    -- "Sujo": nome de marketing de anúncio/online (travessão – ou —)
    ...
    IF p_commit THEN
      INSERT INTO pos_products (
        ...
        v_sku,
        coalesce(NULLIF(r.product_name,''), v_sku),
    ...
    IF v_sample IS NULL OR jsonb_array_length(v_sample) < 10 THEN
      v_sample := coalesce(v_sample,'[]'::jsonb) || jsonb_build_object(
        ...
        'size', v_size, 'color', v_color, 'barcode', v_barcode, 'dirty', v_is_dirty
      );
    END IF;
  END LOOP;
