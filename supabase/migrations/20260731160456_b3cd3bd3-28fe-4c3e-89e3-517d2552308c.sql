    ...
    IF v_ex_id IS NOT NULL THEN
      ...
      IF v_delta <> 0 THEN
        ...
        IF array_length(v_codes,1) IS NOT NULL THEN
          v_prod_id := NULL;
          SELECT p.id, p.store_id, p.stock, p.sku, p.barcode
            INTO v_prod_id, v_prod_store, v_prod_stock, v_prod_sku, v_prod_barcode
          ...
          IF v_prod_id IS NOT NULL THEN
            ...
            INSERT INTO public.pos_stock_adjustments(
              ...
              v_prod_store, v_prod_id, v_prod_sku, v_prod_barcode,
              COALESCE(v_ex_name,''),
