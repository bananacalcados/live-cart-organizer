...
BEGIN
  ...
  -- Nome do contato: deriva SOMENTE de mensagens recebidas (incoming).
  -- O sender_name de mensagens enviadas (outgoing) é o nome da vendedora/atendente
  -- e não pode ser usado como nome do cliente.
  contact_names AS (
  ...
  results AS (
    SELECT
      ...
      -- Para o nome exibido usa apenas o sender_name de mensagens recebidas.
      -- Em grupos, o sender_name do incoming é o participante; mantém esse comportamento.
      cn.sender_name::text AS sender_name,
      l.status,
