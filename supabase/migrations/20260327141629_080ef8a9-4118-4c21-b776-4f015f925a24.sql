DROP FUNCTION IF EXISTS public.get_conversations(uuid, boolean);

CREATE OR REPLACE FUNCTION public.get_conversations(
  p_number_id uuid DEFAULT NULL::uuid,
  p_dispatch_only boolean DEFAULT NULL::boolean
)
RETURNS TABLE(
  phone text,
  last_message text,
  ...
  results AS (
    SELECT
      ...
      (
        ...
        OR (l.direction = 'outgoing' AND hi.phone IS NULL)
      ) AS is_dispatch_only,
      l.channel,
      (hi.phone IS NOT NULL) AS has_incoming
    FROM latest l
    ...
    LEFT JOIN has_in hi ON hi.phone::text = l.phone AND (hi.whatsapp_number_id IS NOT DISTINCT FROM l.whatsapp_number_id)
  )
