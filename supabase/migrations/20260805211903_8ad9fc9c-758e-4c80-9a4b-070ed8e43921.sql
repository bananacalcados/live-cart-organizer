DROP FUNCTION IF EXISTS public.expedition_cancel_sale(uuid, text, boolean);

CREATE OR REPLACE FUNCTION public.expedition_cancel_sale(p_sale_id uuid, p_reason text DEFAULT NULL, p_restore_stock boolean DEFAULT true)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  UPDATE public.pos_sales
     SET status = 'cancelled',
         ...
         expedition_deleted_by = v_uid,
         expedition_deleted_by_email = v_email,
         expedition_deleted_at = now(),
         notes = COALESCE(notes,'') || ' [Excluído da expedição: ' || v_reason
                 || COALESCE(' — por ' || v_email, '') || ']',
         updated_at = now()
   WHERE id = p_sale_id;

  IF v_sale.source_order_id IS NOT NULL THEN
    PERFORM public.forfeit_physical_prize(v_sale.source_order_id, v_reason);
  END IF;

  RETURN jsonb_build_object('ok', true, 'restored_items', v_restored, 'deleted_by', v_email);
END;
$$;