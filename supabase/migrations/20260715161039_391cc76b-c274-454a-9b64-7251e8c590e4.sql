...
-- Ajuste das regras: leitura NÃO qualifica mais como quente
...
BEGIN
  ...
  scored AS (
    SELECT c.*, GREATEST(0, d_total - d_reacted) AS d_ignored,
      ...
      CASE
        ...
        ELSE 'frio'
      END AS temp,
      -- Classificação de disparo NOVA (ajustada)
      CASE
        WHEN d_ign_streak >= v_silence_threshold
          ...
          THEN 'silencio'
        -- QUENTE: compra <=90d OU resposta <=30d (leitura fora)
        WHEN last_purchase_at >= now() - interval '90 days'
             OR last_reply_at >= now() - interval '30 days'
          THEN 'quente'
        -- MORNO: compra 90..180d OU lead sem compra captado <=60d OU leitura <=30d sem compra recente
  ...
  FROM tagged t WHERE cu.id = t.id;

  GET DIAGNOSTICS v_updated = ROW_COUNT;

  SELECT jsonb_object_agg(lead_temperature, cnt) INTO v_counts FROM (
    SELECT lead_temperature, count(*) cnt FROM customers_unified WHERE lead_temperature IS NOT NULL GROUP BY 1
  ) x;
  SELECT jsonb_object_agg(classificacao_disparo, cnt) INTO v_counts_disp FROM (
    SELECT classificacao_disparo, count(*) cnt FROM customers_unified WHERE classificacao_disparo IS NOT NULL GROUP BY 1
  ) y;
...
-- Simulação read-only alinhada às mesmas regras
...
BEGIN
  ...
  WITH sig AS (
    SELECT right(regexp_replace(phone,'\D','','g'),8) AS suffix8, max(created_at) AS last_reply_at
    FROM whatsapp_messages WHERE direction='incoming' AND phone IS NOT NULL AND created_at > now() - interval '180 days'
    GROUP BY 1
  ),
  reads AS (
    SELECT right(regexp_replace(phone,'\D','','g'),8) AS suffix8,
      max(sent_at) FILTER (WHERE status='read') AS last_read_at
    FROM dispatch_recipients WHERE phone IS NOT NULL AND COALESCE(sent_at,created_at) > now() - interval '365 days'
    GROUP BY 1
  ),
  funnel AS (
  ...
  anchors AS (
    SELECT cu.id, cu.phone_suffix8 AS suffix8, cu.total_orders, cu.last_purchase_at,
  ...
  streaks AS (
    SELECT a.id, count(dr.id) AS d_ign_streak
    FROM anchors a
    ...
    SELECT a.id,
      CASE
        ...
        WHEN a.last_purchase_at >= now() - interval '90 days'
          ...
          THEN 'quente'
        WHEN (a.last_purchase_at >= now() - interval '180 days' AND a.last_purchase_at < now() - interval '90 days')
             OR (COALESCE(a.total_orders,0)=0 AND a.funnel_last_at >= now() - interval '60 days')
             OR (a.last_read_at >= now() - interval '30 days'
                 AND (a.last_purchase_at IS NULL OR a.last_purchase_at < now() - interval '90 days'))
          THEN 'morno'
        ELSE 'frio'
      END AS disp
    FROM anchors a
  ...
  SELECT jsonb_build_object(
    ...
    'por_classe', COALESCE(
      (SELECT jsonb_object_agg(disp,cnt) FROM (SELECT disp, count(*) cnt FROM classed GROUP BY 1) x),
      '{}'::jsonb),
    'gerado_em', now()
  ) INTO v_result;
