CREATE OR REPLACE FUNCTION public.enqueue_dispatch_recipients_guarded(p_dispatch_id uuid, p_candidates jsonb, p_tipo_comunicacao text, p_provider text DEFAULT NULL::text, p_overrides jsonb DEFAULT '[]'::jsonb, p_skip_unify boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
UPDATE public.dispatch_history dh
SET total_recipients = (SELECT count(*) FROM public.dispatch_recipients dr WHERE dr.dispatch_id = dh.id)
WHERE dh.status IN ('scheduled','scheduled_paused')
  AND dh.total_recipients IS DISTINCT FROM (SELECT count(*) FROM public.dispatch_recipients dr WHERE dr.dispatch_id = dh.id);