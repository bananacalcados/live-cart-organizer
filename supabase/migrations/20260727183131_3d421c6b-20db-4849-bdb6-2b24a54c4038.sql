ALTER TABLE public.orders
  ...
  ADD COLUMN IF NOT EXISTS release_to_expedition boolean NOT NULL DEFAULT false;

ALTER TABLE public.pos_sales
  ADD COLUMN IF NOT EXISTS payment_on_delivery boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS expected_payment_method text,
  ADD COLUMN IF NOT EXISTS delivery_payment_received_at timestamptz,
  ADD COLUMN IF NOT EXISTS delivery_payment_method text;

CREATE INDEX IF NOT EXISTS idx_pos_sales_payment_on_delivery
  ON public.pos_sales (store_id, expedition_stage) WHERE payment_on_delivery;
...
AFTER INSERT OR UPDATE OF is_paid, paid_externally, stage, release_to_expedition ON public.orders
FOR EACH ROW
