ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS paid_on_site boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_orders_paid_on_site ON public.orders (event_id) WHERE paid_on_site;
...
BEGIN
  -- Pedido finalizado no site (Shopify): conta como pago no módulo Eventos,
  -- mas NUNCA vira venda no PDV / Expedição (evita pedido duplicado).
  IF COALESCE(NEW.paid_on_site, FALSE) THEN
    RETURN NEW;
  END IF;
