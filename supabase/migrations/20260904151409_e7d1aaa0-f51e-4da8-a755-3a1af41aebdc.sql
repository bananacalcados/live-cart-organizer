ALTER TABLE public.live_whatsapp_clicks
  ADD COLUMN IF NOT EXISTS real_phone text,
  ADD COLUMN IF NOT EXISTS divergent boolean NOT NULL DEFAULT false,
...
BEGIN
  v_key := public.event_phone_key(NEW.phone);
  v_code := substring(v_msg from '#([A-Z0-9]{5})\M');
  v_has_phrase := v_msg ~* 'vim da live';

  -- Fast path: sai cedo se não há código, nem frase, nem clique confirmado com este telefone (7 dias)
  ...
  -- ───────────── 1) TELEFONE digitado bate (chave principal) ─────────────
  IF v_key IS NOT NULL THEN
    ...
    IF v_click.id IS NOT NULL THEN
      -- Telefone e código discordam? Se o código pertence a OUTRO clique ainda sem casamento, o código vence.
  ...
  -- ───────────── 2) CÓDIGO curto (reserva: chegou de outro número) ─────────────
  ...
  IF v_has_phrase THEN
    ...
    -- ───────────── 4) Contexto: "veio da Live" sem fbc/fbp ─────────────
...
-- Pós-casamento: memória de atribuição nos DOIS telefones + marca cliques antigos da mesma pessoa como repetidos
...
BEGIN
  IF p_click.fbc IS NOT NULL OR p_click.fbp IS NOT NULL OR p_click.fbclid IS NOT NULL THEN
    ...
    IF p_click.entered_phone IS NOT NULL
       AND public.event_phone_key(p_click.entered_phone) IS DISTINCT FROM public.event_phone_key(p_real_phone) THEN
      PERFORM public.upsert_meta_attribution(
...
END;
$$;
REVOKE ALL ON FUNCTION public.live_zap_after_match(public.live_whatsapp_clicks, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.live_zap_after_match(public.live_whatsapp_clicks, text) TO service_role;
REVOKE ALL ON FUNCTION public.live_zap_match_incoming() FROM PUBLIC, anon, authenticated;