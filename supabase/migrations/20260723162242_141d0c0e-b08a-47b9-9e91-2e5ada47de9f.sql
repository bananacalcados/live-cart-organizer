...
BEGIN
  ...
  prior_reg AS (
    ...
    WHERE r.created_at < p_from
      AND (r.catalog_page_id IS NULL OR r.catalog_page_id NOT IN (SELECT catalog_lead_page_id FROM ev_ids WHERE catalog_lead_page_id IS NOT NULL))
    ...
    SELECT e.*, COALESCE(p.prior_count,0) AS prior_sales, p.first_prior_at,
      ...
      CASE
        WHEN COALESCE(p.prior_count,0) > 0
          OR (c.customer_id IS NOT NULL AND c.first_purchase_at IS NOT NULL AND c.first_purchase_at < p_from)
  ...
  RETURN COALESCE(v_result, jsonb_build_object('events_count',0,'buyers','{}'::jsonb,'non_buyers','{}'::jsonb,'buyer_list','[]'::jsonb,'non_buyer_list','[]'::jsonb));
END;
$function$;

GRANT EXECUTE ON FUNCTION public.events_buyer_origin_matrix_range(timestamptz, timestamptz, text) TO authenticated, service_role;
