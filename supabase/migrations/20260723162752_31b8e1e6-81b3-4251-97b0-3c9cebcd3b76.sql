...
BEGIN
  ...
  cust AS (
    SELECT DISTINCT ON (public.event_phone_key(cu.whatsapp))
           public.event_phone_key(cu.whatsapp) AS pkey,
    ...
    WHERE public.event_phone_key(cu.whatsapp) IS NOT NULL
      AND public.event_phone_key(cu.whatsapp) IN (SELECT pkey FROM everyone)
    ORDER BY public.event_phone_key(cu.whatsapp), cu.total_orders DESC NULLS LAST
  ),
  lead_hit AS (
