...
DECLARE
  ...
  v_changed int;
BEGIN
  -- 1) edges cu_id <-> zoppy id (only active rows)
  ...
  -- 2) label propagation (connected components), label = min cu_id
  CREATE TEMP TABLE _lbl ON COMMIT DROP AS
  SELECT DISTINCT cu_id, cu_id AS lbl FROM _edges;
  ...
  -- 6) consolidate legacy on survivor (max within component = copies, no sum)
  WITH agg AS (
    SELECT s.survivor,
  ...
  -- 7) fill survivor contact from any loser when missing
  WITH best AS (
  ...
  -- 8) merge source_origins onto survivor (all members' origins)
  ...
  -- 9) repoint customer_list_memberships (avoid unique conflict)
  DELETE FROM public.customer_list_memberships cm
  USING _map m
  WHERE cm.customer_id = m.loser
    AND EXISTS (SELECT 1 FROM public.customer_list_memberships c2
                WHERE c2.customer_id = m.survivor AND c2.list_id = cm.list_id);
