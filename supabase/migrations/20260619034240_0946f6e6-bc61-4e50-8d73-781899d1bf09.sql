-- =====================================================================
-- DE-PARA de códigos do Site/Shopify -> estoque interno (pos_products)
-- =====================================================================

-- 1) Lista de trabalho com itens de venda que NÃO casaram com nenhum produto
CREATE TABLE IF NOT EXISTS public.inventory_sale_unmatched_items (
  ...
  CONSTRAINT inventory_sale_unmatched_items_uniq UNIQUE (sale_id, item_code)
);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.inventory_sale_unmatched_items TO authenticated;
GRANT ALL ON public.inventory_sale_unmatched_items TO service_role;

ALTER TABLE public.inventory_sale_unmatched_items ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated full access inventory_sale_unmatched_items"
  ON public.inventory_sale_unmatched_items
  FOR ALL TO authenticated
  USING (true) WITH CHECK (true);

CREATE TRIGGER update_inventory_sale_unmatched_items_updated_at
  BEFORE UPDATE ON public.inventory_sale_unmatched_items
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- 2) Função de baixa com casamento robusto + de-para
...
BEGIN
  ...
  LOOP
    ...
    -- Códigos candidatos: o GTIN do Site pode vir no barcode OU no sku do item
    v_codes := ARRAY(
    ...
    -- De-para: traduz códigos do Site -> sku/tiny_id internos via apelidos
    SELECT
    ...
    -- Idempotência: já existe baixa desta venda para esse código?
    IF EXISTS (
    ...
    -- D) Qualquer loja (sem exigir saldo)
    IF v_product.id IS NULL THEN
      SELECT id, store_id, stock, sku, barcode, tiny_id INTO v_product
      FROM public.pos_products
      WHERE (
    ...
    -- Sem correspondência -> registra na lista de de-para (worklist)
    IF v_product.id IS NULL THEN
