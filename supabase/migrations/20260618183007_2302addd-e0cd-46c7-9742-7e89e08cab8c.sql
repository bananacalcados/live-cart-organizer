...
BEGIN
  ...
  WITH agg AS (
    SELECT s.survivor, max(cu.legacy_orders) AS lo, max(cu.legacy_spent) AS ls,
      min(cu.legacy_first_purchase_at) AS lf, max(cu.legacy_last_purchase_at) AS ll
    FROM _lbl l JOIN _surv s ON s.lbl = l.lbl
    JOIN public.customers_unified cu ON cu.id = l.cu_id GROUP BY s.survivor
  )
  ...
  -- list memberships: keep only one row per (resolved survivor, list)
  WITH resolved AS (
