...
BEGIN
  ...
  LOOP
    v_sku := NULLIF(btrim(coalesce(rec.sku,'')), '');
    v_gtin := NULLIF(btrim(coalesce(rec.gtin,'')), '');
    ...
    IF v_master_id IS NULL THEN
      ...
      VALUES (
        coalesce(v_sku, 'AUTO-' || substr(md5(rec.base_name), 1, 10)),
      ...
      VALUES (v_run_id, 'create_master', v_master_id, rec.base_name, rec.norm_color, rec.norm_size,
              jsonb_build_object('sku', v_sku, 'tiny_id', rec.tiny_id));
    ELSE
    ...
    IF v_variant_id IS NULL AND v_gtin IS NOT NULL THEN
      SELECT pv.id INTO v_variant_id FROM product_variants pv
        WHERE pv.master_id = v_master_id AND pv.gtin = v_gtin LIMIT 1;
    END IF;
    ...
    IF v_variant_id IS NULL THEN
      ...
      IF v_sku IS NOT NULL THEN
        SELECT pv.master_id INTO v_sku_owner FROM product_variants pv WHERE pv.sku = v_sku LIMIT 1;
      END IF;
      IF v_gtin IS NOT NULL AND v_sku_owner IS NULL THEN
        SELECT pv.master_id INTO v_gtin_owner FROM product_variants pv WHERE pv.gtin = v_gtin LIMIT 1;
      END IF;
      ...
      IF v_sku_owner IS NOT NULL THEN
        ...
        VALUES (v_run_id, 'skip_sku_conflict', v_master_id, rec.base_name, rec.norm_color, rec.norm_size,
                jsonb_build_object('sku', v_sku, 'gtin', v_gtin, 'existing_master_id', v_sku_owner));
      ELSIF v_gtin_owner IS NOT NULL THEN
        v_skipped_gtin_conflict := v_skipped_gtin_conflict + 1;
        INSERT INTO catalog_sync_log(run_id, operation, master_id, base_name, color, size, details)
        VALUES (v_run_id, 'skip_gtin_conflict', v_master_id, rec.base_name, rec.norm_color, rec.norm_size,
                jsonb_build_object('sku', v_sku, 'gtin', v_gtin, 'existing_master_id', v_gtin_owner));
      ELSE
        INSERT INTO product_variants (master_id, sku, gtin, color, size, initial_stock, is_active)
        VALUES (v_master_id, v_sku, v_gtin, rec.norm_color, rec.norm_size, 0, true)
        ...
        VALUES (v_run_id, 'create_variant', v_master_id, v_variant_id, rec.base_name, rec.norm_color, rec.norm_size,
                jsonb_build_object('sku', v_sku, 'gtin', v_gtin));
      END IF;
    ELSE
      UPDATE product_variants SET
        sku = CASE WHEN coalesce(sku,'') = '' THEN v_sku ELSE sku END,
        gtin = coalesce(gtin, v_gtin),
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_gtin_conflict', v_skipped_gtin_conflict,
    'offset', p_offset,
    'limit', p_limit
  );
END $$;
