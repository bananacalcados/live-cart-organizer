DROP FUNCTION IF EXISTS public.enqueue_dispatch_recipients_guarded(uuid, jsonb, text, text, jsonb);
DROP FUNCTION IF EXISTS public.enqueue_dispatch_recipients_guarded(uuid, jsonb, text, text, jsonb, boolean);
...
CREATE OR REPLACE FUNCTION public.enqueue_dispatch_recipients_guarded(
  ...
  p_override_quota boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_name text;
  v_was_null boolean;
  -- Motivos "macios" que a liberação manual (furar teto) pode ignorar.
  -- NUNCA inclui opt_out / banido / silencio / sem_telefone / merged.
  v_soft_reasons text[] := ARRAY['cota_estourada','cooldown_ativo','tipo_nao_permitido','classe_sem_regra','cota_zero'];
BEGIN
  ...
  IF p_skip_unify THEN
    ...
    UPDATE public.dispatch_history SET
      ...
      total_recipients = (SELECT count(*) FROM public.dispatch_recipients dr2 WHERE dr2.dispatch_id = p_dispatch_id)
  ...
  ins AS (
    ...
    SELECT
      ...
      q.unified_id,
      CASE
        WHEN ov.uid IS NOT NULL THEN concat('override:', COALESCE(ov.motivo,'sem_motivo'), '|', q.reason)
        WHEN NOT q.eligible AND p_override_quota AND q.reason = ANY(v_soft_reasons)
          THEN concat('override:quota_bypass|', q.reason)
        ELSE NULL
      END
    FROM q
    ...
    WHERE q.phone IS NOT NULL
      AND (
        q.eligible
        OR ov.uid IS NOT NULL
        OR (p_override_quota AND q.reason = ANY(v_soft_reasons))
      )
    ON CONFLICT (dispatch_id, phone) DO NOTHING
  ...
  IF p_override_quota THEN
    v_overrides_meta := v_overrides_meta || jsonb_build_array(
      jsonb_build_object(
        'motivo', 'quota_bypass_manual',
        'override_by', v_actor,
        'override_at', now()
      )
    );
  ...
  UPDATE public.dispatch_history SET
    ...
    quota_check_summary = v_summary
      || jsonb_build_object('auto_created_unified', v_auto_created, 'quota_override', p_override_quota),
  ...
  RETURN jsonb_build_object(
    ...
    'quota_override', p_override_quota,
    'summary', v_summary
  );
END;
$function$;

REVOKE ALL ON FUNCTION public.enqueue_dispatch_recipients_guarded(uuid, jsonb, text, text, jsonb, boolean, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.enqueue_dispatch_recipients_guarded(uuid, jsonb, text, text, jsonb, boolean, boolean) TO authenticated, service_role;