...
BEGIN
  ...
  WHERE cv.phone_suffix8 IS NOT NULL
    ...
    -- Teto global de marketing (anti-spam entre TODAS as automações já enviadas)
    AND NOT EXISTS (
    ...
    -- Teto global incluindo envios EM VOO (pendente) de qualquer campanha de carrossel,
    -- para que duas campanhas no mesmo dia não enfileirem o mesmo cliente duas vezes
    AND NOT EXISTS (
      SELECT 1 FROM public.campanha_envios ce2
      WHERE ce2.phone_suffix8 = cv.phone_suffix8
        AND ce2.status = 'pendente'
        AND COALESCE(ce2.enviado_em, ce2.criado_em, now()) >= now() - (p_global_cap_days || ' days')::interval
    )
