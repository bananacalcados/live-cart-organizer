...
DECLARE
  ...
  v_180_days_ago timestamptz := v_now - interval '180 days';
  ...
  v_result jsonb;
BEGIN
  -- Step 1: Get all candidates (inactive 45-180 days, have phone, have orders)
  ...
  WHERE zc.phone IS NOT NULL
    ...
    AND zc.last_purchase_at >= v_180_days_ago;

  SELECT count(*) INTO v_total_candidates FROM _candidates;

  -- Step 2: Mark blocked by cooldown (outgoing messages within segment-specific window)
  ...
  -- Step 3: Mark blocked by incoming messages (active conversation in last 30 days)
  ...
  -- Step 4: Mark blocked by active chat assignment
  ...
  -- Step 5: Mark blocked by recent purchase (pos_sales in last 30 days)
  ...
  SELECT jsonb_build_object(
    ...
    'customers', COALESCE((
      ...
      FROM (
        SELECT
          c.first_name, c.last_name, c.phone, c.rfm_segment,
          c.rfm_total_score, c.last_purchase_at, c.total_spent,
          c.avg_ticket, c.total_orders, c.preferred_style,
          c.shoe_size, c.cashback_balance, c.tags
        FROM _candidates c
        WHERE c.id NOT IN (SELECT id FROM _blocked_cooldown)
          AND c.id NOT IN (SELECT id FROM _blocked_incoming)
          AND c.id NOT IN (SELECT id FROM _blocked_chat)
