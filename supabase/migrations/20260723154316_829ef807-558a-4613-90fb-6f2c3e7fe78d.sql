...
BEGIN
  ...
  WITH
  -- Todos os pedidos do evento (Kanban)
  ev_orders AS (
    SELECT
      o.id AS order_id,
  ...
  -- Compradores = pedidos pagos do evento, deduped por telefone
  buyers_raw AS (
  ...
  -- Não-compradores = pedidos NÃO pagos do evento (só orders — não leads, não comentários)
  nb_raw AS (
    SELECT
  ...
  -- Vendas anteriores (pos_sales antes do evento, excluindo o próprio evento)
  prior_ps AS (
  ...
  -- Pedidos anteriores pagos no Kanban (outros eventos)
  prior_ord AS (
  ...
  classified AS (
    SELECT e.*, COALESCE(p.prior_count,0) AS prior_sales, p.first_prior_at,
      ...
      CASE
        -- Já era cliente: tem compra anterior ou registro no CU com pelo menos 1 compra anterior ao evento
        ...
        -- Era lead antes: aparecia em qualquer fonte de lead ANTES do evento
