...
CREATE OR REPLACE FUNCTION public.apply_pos_sale_item_stock_movement()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'extensions'
...
BEGIN
  ...
  PERFORM public.process_pos_sale_sale_event(NEW.sale_id);
  RETURN NEW;
END;
$function$;

DROP TRIGGER IF EXISTS trg_pos_sale_items_stock_movement ON public.pos_sale_items;
CREATE TRIGGER trg_pos_sale_items_stock_movement
AFTER INSERT ON public.pos_sale_items
FOR EACH ROW
EXECUTE FUNCTION public.apply_pos_sale_item_stock_movement();