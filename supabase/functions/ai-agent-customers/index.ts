...
serve(async (req) => {
  ...
  try {
    ...
    const { novidades, verba, meta, customPrompt } = body;

    // 1. Fetch priority customers from zoppy_customers
    const now = new Date();
    const sixMonthsAgo = new Date(now.getTime() - 180 * 24 * 60 * 60 * 1000);

    // Fetch customers by priority segments
    const { data: customers, error: custError } = await supabase
      ...
      .not('phone', 'is', null)
      .gt('total_orders', 0)
      .order('rfm_total_score', { ascending: true })
      .limit(500);
    ...
    const priorityCustomers = (customers || [])
      ...
      .filter(c => {
        // Priority: Em Risco, Hibernando, Não Pode Perder, Quase Dormindo, Precisam Atenção
        const prioritySegments = ['Em Risco', 'Hibernando', 'Não Pode Perder', 'Quase Dormindo', 'Precisam Atenção', 'Perdidos'];
        return prioritySegments.includes(c.rfm_segment) || c.days_inactive >= 30;
      })
      .sort((a, b) => {
        // Sort: highest value at risk first
        ...
        const aPrio = segmentPriority[a.rfm_segment] || 7;
        const bPrio = segmentPriority[b.rfm_segment] || 7;
        if (aPrio !== bPrio) return aPrio - bPrio;
        return b.total_spent - a.total_spent;
      })
      .slice(0, 100);

    // 2. Get segment counts for context
    ...
    const userPrompt = customPrompt || `Analise a base de clientes da Banana Calçados e entregue um plano de reativação para esta semana.

RESUMO DA BASE POR SEGMENTO:
${Object.entries(segmentSummary).map(([seg, count]) => `- ${seg}: ${count} clientes`).join('\n')}

DADOS DOS CLIENTES PRIORITÁRIOS (${priorityCustomers.length} clientes em risco ou inativos):
${JSON.stringify(priorityCustomers.slice(0, 50), null, 2)}

CONTEXTO DA SEMANA:
- Data de hoje: ${today}
- Novidades em estoque: ${novidades || 'Não informado'}
- Verba disponível para ações desta semana: R$ ${verba || 'Não informado'}
- Meta de faturamento desta semana: R$ ${meta || 'Não informado'}
