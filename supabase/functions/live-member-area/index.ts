...
Deno.serve(async (req) => {
  ...
  try {
    ...
    /**
     * Frete já definido pela EQUIPE no modal do pedido da Live (frete grátis,
     * valor de frete, retirada/mototáxi). Nesses casos a cliente não precisa
     * escolher forma de envio de novo na área de membros.
     */
    function teamShippingDefined(order: any) {
      const info: any = order?.shipping_info || null;
      return !!(
        info?.method ||
    ...
    /** A cliente escolheu o frete na área de membros (ou a equipe já definiu)? */
    function shippingChosen(order: any) {
      return (order?.shipping_info as any)?.source === "member_area" || teamShippingDefined(order);
    }
    ...
    if (action === "identify") {
      ...
      const event = await resolveCurrentEvent();
      const lookup = async (eventId: string | null) => {
        const { data, error } = await supabase.rpc("member_area_find_by_identity", {
          p_event_id: eventId,
          p_kind: kind,
          p_value: value,
        });
        if (error) console.error("[member-area] identify falhou:", error);
        return Array.isArray(data) ? data[0] : data;
      };
      // Primeiro no evento atual; se não achar, procura em qualquer pedido
      // aberto dela (pedido montado numa live anterior).
      let row = await lookup(event?.id || null);
      if (!row && event?.id) row = await lookup(null);
