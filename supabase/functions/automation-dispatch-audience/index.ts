...
serve(async (req) => {
  ...
  try {
    ...
    if (presetKeys.length > 0 && (audienceSource === 'rfm' || audienceSource === 'both')) {
      ...
      rfmData = await fetchAllRows(supabase, 'crm_customers_v', 'phone, first_name, last_name, email, city, state, rfm_segment, region_type, gender, ddd, avg_ticket, total_orders, last_purchase_at, rfm_recency_score, opt_out_mass_dispatch');
    } else if (audienceSource === 'rfm' || audienceSource === 'both' || audienceSource === 'crm') {
      if (audienceSource === 'crm' || rfmSelectAll) {
        rfmData = await fetchAllRows(supabase, 'crm_customers_v', 'phone, first_name, last_name, email, city, state, rfm_segment, region_type, gender, opt_out_mass_dispatch');
