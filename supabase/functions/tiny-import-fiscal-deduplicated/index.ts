...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const limit: number = Math.min(Math.max(Number(body?.limit) || 50, 1), 300);
    const onlyMethod: string | null = body?.only_method || null; // 'gtin' | 'fallback_name_sku' | null
    ...
    const { data: run } = await supabase.from("tiny_import_runs").insert({
      ...
      stats: { limit, only_method: onlyMethod, skip_imported: skipImported },
    }).select("id").single();
    const runId = run!.id;
    ...
    const stats = {
      ...
      with_gtin: 0,
      ignored_no_token: 0,
      skipped_no_tiny_id: 0,
    };

    // Cache master lookups by sku_root within this run
    ...
    for (const row of dedupRows || []) {
      ...
      // Fetch from Tiny (with retry once on rate limit)
      ...
      if (!masterId) {
        ...
        if (existing) {
          ...
          const { data: ins, error: insErr } = await supabase.from("products_master").insert({
          ...
          if (insErr) {
            await supabase.from("tiny_import_errors").insert({
              run_id: runId, dedup_index_id: row.id,
              error_code: "master_insert_fail", error_message: insErr.message,
            });
            await new Promise((r) => setTimeout(r, 350));
            continue;
          }
        ...
        masterCache.set(sku_root, masterId);
        stats.masters_upserted++;
      }

      // Variant upsert (by master_id+color+size or sku)
      ...
      const variantPayload: any = {
        ...
        sale_price_override: sale,
        weight_kg_override: weight,
        is_active: true,
        tiny_variant_id: String(tinyId),
        tiny_imported_at: new Date().toISOString(),
      };

      // Try match by master+color+size first
      ...
      // Mark dedup as imported
      await supabase.from("product_dedup_index").update({
        imported_at: new Date().toISOString(),
      }).eq("id", row.id);

      // Rate limit: ~2 req/s para Tiny ser feliz
      await new Promise((r) => setTimeout(r, 350));
    }
